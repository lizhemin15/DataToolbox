...
function switchDetailTab(tab) {
    const previewContent = document.getElementById('previewContent');
    const sqlWorkbench = document.getElementById('sqlWorkbench');
    const tabPreview = document.getElementById('tabPreview');
    const tabSql = document.getElementById('tabSqlWorkbench');
    
    if (tab === 'preview') {
        previewContent.style.display = '';
        sqlWorkbench.style.display = 'none';
        tabPreview.classList.add('active');
        tabSql.classList.remove('active');
    } else {
        previewContent.style.display = 'none';
        sqlWorkbench.style.display = 'flex';
...
function autoFillSqlEditor() {
    const editor = document.getElementById('sqlEditor');
    if (!editor || editor.value.trim()) return;
    
    const dbInfo = window._currentDbInfo;
    const tableName = currentPreviewTable;
...
function quoteTableName(name, dbType) {
    switch (dbType) {
        case 'postgresql': case 'timescaledb': case 'cockroachdb':
            return `"${name}"`;
        case 'sqlserver':
            return `[${name}]`;
        case 'clickhouse':
...
async function executeSqlWorkbench() {
    const editor = document.getElementById('sqlEditor');
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE}/api/v1/databases/${dbInfo.id}/sql`, {
            method: 'POST',
        ...
        if (data.success) {
            ...
            document.getElementById('sqlResultInfo').textContent = 
                `${rowCount} 行 ${elapsed ? '· ' + elapsed : ''}`;
            ...
            document.getElementById('sqlResultInfo').textContent = '执行失败';
        ...
        document.getElementById('sqlResultInfo').textContent = '请求失败';
        document.getElementById('sqlResultTable').innerHTML = 
...
function renderSqlResult(columns, rows) {
    ...
    if (!columns || columns.length === 0) {
        container.innerHTML = '<div style="padding:16px;color:#64748b;">查询成功，无返回数据</div>';
        return;
    }
    if (!rows || rows.length === 0) {
        container.innerHTML = '<div style="padding:16px;color:#64748b;">0 行结果</div>';
        return;
    }
...
function addSqlHistory(sql) {
    ...
    if (sqlHistory.length > MAX_SQL_HISTORY) sqlHistory.pop();
    renderSqlHistory();
}
...
function renderSqlHistory() {
    ...
    if (sqlHistory.length === 0) {
        container.innerHTML = '<div style="font-size:12px;color:#94a3b8;padding:4px 8px;">暂无历史</div>';
        return;
    }
    container.innerHTML = sqlHistory.map((sql, i) => 
        `<div class="sql-history-item" onclick="loadSqlFromHistory(${i})" title="${escapeHtml(sql)}">${escapeHtml(sql.substring(0, 80))}${sql.length > 80 ? '...' : ''}</div>`
    ).join('');
}

function loadSqlFromHistory(index) {
    if (index >= 0 && index < sqlHistory.length) {
        document.getElementById('sqlEditor').value = sqlHistory[index];
    }
}
...
function formatSql() {
    ...
    // 主要子句换行
    sql = sql.replace(/\b(SELECT|FROM|WHERE|ORDER BY|GROUP BY|HAVING|LIMIT|OFFSET|LEFT JOIN|RIGHT JOIN|INNER JOIN|JOIN|UNION|INSERT INTO|VALUES|SET|UPDATE|DELETE FROM)\b/gi, '\n$1');
...
// 监听表选择，自动切换 tab 标题
const _origPreviewTable = previewTable;
previewTable = function(tableName) {
    _origPreviewTable(tableName);
    currentPreviewTable = tableName;
};
...
async function loadPage(dir) {
    ...
    await reloadTableData(dbInfo.id, tableName);
}

function changePageSize() {
    const sel = document.getElementById('pageSizeSelect');
    if (!sel) return;
    currentPageSize = parseInt(sel.value) || 100;
    currentPage = 1;
    
    const dbInfo = window._currentDbInfo;
    const tableName = currentPreviewTable;
    if (dbInfo && tableName) {
        reloadTableData(dbInfo.id, tableName);
    }
}

async function reloadTableData(dbId, tableName) {
    const previewContent = document.getElementById('previewContent');
    previewContent.innerHTML = '<div style="text-align:center;padding:60px;">加载中...</div>';
    
    try {
        const response = await fetchWithAuth(
            `${API_BASE}/api/v1/databases/${dbId}/tables/${encodeURIComponent(tableName)}?page=${currentPage}&size=${currentPageSize}`
        );
        const data = await response.json();
        ...
        if (data.success) {
            ...
            if (dataTable) {
                ...
                const columns = Array.from(dataTable.querySelectorAll('thead th')).map(th => th.textContent.replace(/\n.*/, '').trim()).filter(c => c !== '操作');
                
                const hasData = data.data && data.data.length > 0;
                tbody.innerHTML = hasData ? data.data.map((row, rowIndex) => {
                    const rowId = 'row-' + Date.now() + '-' + rowIndex;
                    return `
                        <tr data-row-id="${rowId}" data-row-index="${rowIndex}">
                            ${columns.map(col => {
                                const displayValue = formatCellValue(row[col]);
                                return `<td data-column="${escapeHtml(col)}" class="editable-cell">${displayValue.html}</td>`;
                            }).join('')}
                        </tr>
                    `;
                }).join('') : `<tr class="empty-row"><td colspan="${columns.length}" style="text-align:center;color:#718096;padding:20px;">暂无数据</td></tr>`;
            }
            
            document.getElementById('tableRowCount').textContent = `${data.data?.length || 0} 行`;
...
// 表格工具栏 CSS
const tableToolbarStyle = document.createElement('style');
tableToolbarStyle.textContent = `
.table-toolbar {
    display: flex;
...
`;
document.head.appendChild(tableToolbarStyle);
