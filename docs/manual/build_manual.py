...
def build_other_modules(doc):
    ...
    bullet(doc, '1. 规则配置（规则树）：按 NM（6 位编码）/ XH（层级编码）/ 名称 / 类别 / SQL / 参数维护规则，'
                '支持下载模板后批量粘贴导入或从 Excel 导入，SQL 用 Oracle 方言编写并由平台自动转换方言；'
    ...
    image(doc, '15-quality.png', '图 18　数据质量审核（规则配置）')
    ...
    image(doc, '16-apps.png', '图 19　应用广场')
    ...
    body(doc, '大屏编辑器用于拼装数据大屏：从组件库中拖入图表、表格、指标卡等组件，绑定数据源后'
              ...
              '该模块默认隐藏，可在“设置 → 标签页设置”中开启。')
    image(doc, '17-screen-editor.png', '图 20　大屏编辑器')
