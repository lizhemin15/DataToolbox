...
async function ensureGovernanceScriptsLoaded() {
    await loadLazyScript('gov-shared.js?v=2026.05.10.083043.083043');
    await loadLazyScript('gov-api.js?v=2026.05.10.083043.083043');
    await loadLazyScript('governance.js?v=2026.05.10.083043.083043');
}

async function ensureQualityAuditScriptLoaded() {
    await loadLazyScript('quality-audit.js?v=2026.05.10.083043.083043');
}
...
async function handleSendAiMessage() {
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE}/api/v1/agent/query`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function executeConfirmedSQL(confirmId, sql, dbId, messageId) {
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE}/api/v1/agent/confirm-execute`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function generateImportCodeWithAI() {
    ...
    try {
        const response = await fetchWithAuth(`${API_BASE}/api/v1/agent/codegen`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function showAddVectorModal() {
    // 先获取该数据库的表列表
    try {
        const response = await fetchWithAuth(`${API_BASE}/api/v1/retrieval/tables?db_id=${currentDb.id}`);
        const data = await response.json();
