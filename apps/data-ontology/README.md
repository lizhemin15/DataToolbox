...
所有用户信息和数据库配置都会自动保存到 `apps/data-ontology/data-store.json` 文件中。
...
### 后续运行
- 自动从 `data-store.json` 加载用户和数据库配置
- 所有修改（添加、编辑、删除数据库配置）都会自动保存

### 数据迁移
如果需要迁移到新环境：
1. 复制整个 `apps/data-ontology/` 文件夹
2. 或者只复制 `apps/data-ontology/data-store.json` 文件
3. 在新环境启动服务器即可自动加载所有配置

### 数据备份
建议定期备份 `apps/data-ontology/data-store.json` 文件，以防数据丢失。

📖 **详细说明**：查看 [PERSISTENCE.md](PERSISTENCE.md) 了解更多关于持久化存储的详细信息。
