...
func governancePresetDefinitions() map[string]GovernanceTask {
	...
	return map[string]GovernanceTask{
		...
		"产品汇总Word生成（套用表格模板）": {
			Owner:         "admin",
			Name:          "产品汇总Word生成（套用表格模板）",
			Type:          "interactive",
			Description:   "上传「表格模板」Word 与「纯文本产品介绍」Word，按「省-市-区」把每个产品转成一张套用模板样式的表格，输出汇总 Word",
			JsCode:        loadGovernancePresetJS("gov-product-word-tables.js"),
			InputType:     "file",
			AcceptExts:    []string{".docx", ".doc", ".wps"},
			FileBatchMode: "multi",
			RunMode:       "frontend",
			ExecutionMode: "frontend",
			ExampleFiles: []GovernanceExampleFile{
				{Name: "表格模板.docx", Path: "表格模板.docx"},
				{Name: "纯文本产品介绍.docx", Path: "纯文本产品介绍.docx"},
			},
			CreatedAt: now,
			Status:    "idle",
		},
